...
import {
  ...
  Shield, LogOut, History, Sparkles, TrendingUp
} from "lucide-react";
...
interface ProfileData {
  user: { name: string | null; email: string | null; image: string | null; credits: number; plan: string; createdAt: string };
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    fetch("/api/profile").then(r => r.json()).then(d => setData(d)).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  const credits = (session?.user as any)?.credits ?? data?.user?.credits ?? 0;
  ...
  return (
    <div className="min-h-screen" style={{ background: "#f7f7f9" }}>
      ...
      <div
        ...
        style={{
          height: "180px",
          background: "linear-gradient(135deg, #4c1d95 0%, #7c3aed 45%, #4f46e5 100%)",
        }}
      >
        {/* Decorative dots */}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-300 mono mt-1">
                  Thành viên từ {data?.user?.createdAt ? new Date(data.user.createdAt).toLocaleDateString("vi-VN") : "—"}
                </p>
              </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
          {[
            ...
            },
            {
              ...
              value: (data?.totalCreditsUsed ?? 0).toLocaleString(),
              sub: "tổng cộng",
            },
          ].map((stat, i) => (
            <div key={i} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm flex flex-col gap-3">
              ...
              <div>
                ...
                {stat.href ? (
                  <Link href={stat.href} className="text-xs font-bold flex items-center gap-1 mt-1"
                    style={{ color: "#7c3aed" }}>
